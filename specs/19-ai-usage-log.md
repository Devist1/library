...
- **总使用次数**：**16 次**（第 1 次 ~ 第 16 次）
- **最近一次整理**：2026-09-24 —— 原第 7 次（`05` v1.0 成文）与原第 8 次（`05` v1.1 审查补正）**合并为第 7 次**，其后条目顺延（原 9~14 → 8~13），并补登第 14 次（`17` / `18` 生成）、第 15 次（本文件整理）、第 16 次（登记人类裁决 2 与回写）
- **使用工具**：全部为 **CodeBuddy**
- **人类裁决（已回写）**：① 2026-09-24 保留四个学生子类（`H-2` 退化不采纳）；② 2026-09-24 配置化边界**选项 A**（仅借阅与罚款规则入配置表，其余为需求常量 + 配置项 / 单一入口）—— 见第 16 次与 `17` §12
- **推送状态**：本地 `main` 领先 `origin/main` **3 个提交**（`170097a` / `527acc8` / `558cfd0`），第 16 次（本文件）尚未提交
...
| 15 | 留痕 | 本文件 | 合并第 7 / 8 次、编号顺延、补登第 14 次、新增开头总览、登记「保留四个学生子类」裁决 | `527acc8` |
| 16 | 详细设计 + 留痕 | `17` + `07` §10.2 + `13` §2 + 本文件 | 登记人类裁决 2（配置化边界选项 A）并回写：`17` 关闭 `RISK-REQ-12`、`07` §10.2 增「配置化边界」引用块、`13` §2 增约束行、`17` §12 增裁决 2 与实现约束 5 条 | `558cfd0`（`17` / `07` / `13`）；本文件**未提交** |
...
（1）按人类裁决「**配置化边界选项 A**」回写相应文档 —— `17-risk-analysis.md`（关闭 `RISK-REQ-12`、§12 增裁决 2 与实现约束）、`07-architecture.md` §10.2（增「规则数值须配置化的适用边界」引用块）、`13-database-design.md` §2（增「配置化边界」约束行）；（2）执行 `git add specs` + `git commit -m "revise architecture specs after human review"`；（3）继续完成未完成的留痕任务（本文件登记本次回写与裁决）。

### Agent 修改文件

- `specs/17-risk-analysis.md`（v1.0.1 → **v1.0.2**）
- `specs/07-architecture.md`（§10.2 新增引用块）
- `specs/13-database-design.md`（§2 新增约束行）
- `specs/19-ai-usage-log.md`（本文件：总览更新至 16 次、新增第 16 次、遗留事项更新）
- 前三个文件已随提交 `558cfd0` 落盘；本文件**未提交**

### 输出摘要

1. **登记人类裁决 2**（2026-09-24，配置化边界**选项 A**）：第七条第 3 款适用对象 = 借阅与罚款规则（`borrow_policies` / `fine_rules`）；50 元封顶、保留 3 天、预约上限 3、会话 30 分钟为**需求常量**，以配置项或领域层单一入口承载，**不新增配置表**。
2. **`17` 回写**：`RISK-REQ-12` 由**高**降为**低**并关闭；§9 第 2 项标注已裁决且回写完成；§12 新增裁决 2 一行与「裁决 2 的实现约束」5 条（入表 / 不入表清单、单一入口、禁散落硬编码、禁落 SQL、种子脚本例外）；文件头版本 v1.0.2 与附录 B v1.0.2 修订行。
3. **`07` §10.2 回写**：在 `RULE-13` 不归属清单后新增引用块 5 条，明确入配置表与不入配置表的对象、四类常量的承载与判定入口，并声明 RULE-13 的禁止要求不变。
4. **`13` §2 回写**：新增「配置化边界」约束行 —— 不新增配置表，四类常量禁止出现在 `CHECK` / 触发器 / 视图中。
5. **提交**：`558cfd0` revise architecture specs after human review（3 files changed, 24 insertions(+), 4 deletions(-)）。
6. **本文件留痕**：总览「总使用次数」更新为 **16 次**、新增精简更改内容第 16 行、新增第 16 次记录、遗留事项 #21 / #23 / #35 / #36 更新并新增 #37 / #38。

### 人工审查结果

（待本人填写：审查结论与修改意见）

> 说明：本条同时包含**按裁决回写设计文档**与**留痕**两类动作；回写内容以人类裁决为依据，未新增或改变任何业务口径。
...
`558cfd0` revise architecture specs after human review（**已按人类指令提交**，含 `17` / `07` / `13`；本文件 `19` 的本次更新**未提交**）。本地 `main` 领先 `origin/main` 3 个提交（`170097a` / `527acc8` / `558cfd0`），推送须由本人明确指令后执行。
...
| 21 | 本文件第 1~**15** 次条目**已提交**（`60a1c22` / `eb4bb6a` / `170097a` / `527acc8`）；第 16 次的 `17` / `07` / `13` 已随 `558cfd0` 提交，**本文件（第 16 次记录）尚未提交** | 第五条第 6 款（Agent 不得自行提交） | 待人类提交 |
| 22 | 使用工具**统一为 CodeBuddy**（同 #9）：头部清单与各条目已一并统一，原 `Claude Code / Codex / OpenCode` 列举不再保留 | 第四条第 5 款、第十三条 | **已处理** |
| 23 | 后续每次生成（`09-design-model.md`、`10`~`12` 时序图、 `14-api-spec.md` / `15-test-plan.md` / `16-tasks.md` 及代码、测试）须在本文件续登一条记录；编号自**第 17 次**起顺延（`17` / `18` 已由第 14 次登记） | `E3`、第四条第 5 款 | 持续 |
...
| 36 | ~~第 14、15 次记录尚未提交~~ → **已提交**（`170097a` / `527acc8`）。当前未提交者：第 16 次的本文件更新 | 第五条第 6 款 | **已提交**（本文件除外） |
| 37 | 2026-09-24 人类裁决「**配置化边界选项 A**」已回写三处：`17` §12 裁决 2 + `RISK-REQ-12` 关闭、`07` §10.2 引用块、`13` §2 约束行（提交 `558cfd0`）；**待补**：`15-test-plan.md` 成文时加「四类常量不得散落硬编码」的检查 | 第十三条第 2 款、`17` §12 | **已回写**，`15` 待成文 |
| 38 | 本地 `main` 领先 `origin/main` **3 个提交**（`170097a` / `527acc8` / `558cfd0`）；推送须由本人明确指令后执行（第五条第 6 款、第十一条第 2 款） | 第五条第 6 款 | 待人类推送 |

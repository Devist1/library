...
| 配置化边界（2026-09-24 人类裁决，选项 A） | 第七条第 3 款的适用对象 = **借阅与罚款规则**（`borrow_policies` / `fine_rules`）；**50 元封顶、预约保留 3 天、单人预约上限 3、会话 30 分钟**为**需求常量** —— 以配置项或领域层单一入口承载，**不新增配置表**，且**禁止**出现在任何 `CHECK` / 触发器 / 视图中 | `07` §10.2、`17` §12 裁决 2 |
